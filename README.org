...
*Description*: Tool calls are synchronous, so once a background job is detached the agent normally has to poll. This extension listens on =$XDG_RUNTIME_DIR/pi-inbox/<pid>.sock= and exports the path as =$PI_INBOX= to every tool subprocess. Each connection becomes one message (JSON ={"source": "...", "text": "..."}= or plain text, 16 KB cap) that is delivered right away when the agent is idle, otherwise as a follow-up after the current run, and rendered as a compact =▌inbox <source>= block.
