...
*Requirements:*
- =diff= npm package (automatically installed via =npm install= — see Quick Start)
- Optional: =delta=, =nvim=, or =vim= for external diff viewing

*Note:* The =diff= package provides proper unified diff generation (Myers algorithm). Without it, slow-mode will fail to load.
...
#+begin_quote
💡 Inspired by patterns from [[https://github.com/Mic92][Mic92]]'s pi extensions. Thanks Mic92!
#+end_quote
...
The =slow-mode= extension requires the =diff= package for proper unified diff generation:
...
This installs the =diff= package (v7+) specified in =package.json=.
...
- [[https://github.com/Mic92][Mic92]] — Original direnv extension implementation ([[https://github.com/Mic92/dotfiles/blob/main/home/.pi/agent/extensions/direnv.ts][source]]) and inspiration for questionnaire patterns
