...
*Description*: Registers a native =fetch= tool that the LLM can use for HTTP requests without relying on =bash= or =curl=. Uses Node.js built-in =fetch= — no external dependencies.
